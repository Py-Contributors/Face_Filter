...
def test_app():
    ...
    response = app.test_client().post('/facedetection', data=data, content_type="multipart/form-data")
    assert response.status_code == 200
    assert type(response.data) == bytes
    ...
    response = app.test_client().post('/facefilter', data=data, content_type="multipart/form-data")
    assert response.status_code == 200
    assert type(response.data) == bytes
